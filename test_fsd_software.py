...
class TestFSDSoftware(unittest.TestCase):
    ...
    # New tests
    def test_adjust_speed_just_below_max_limit(self):
        """Test adjusting the vehicle's speed to just below the maximum limit."""
        result = self.fsd.adjust_vehicle_speed(119)
        self.assertEqual(result, 119)

    def test_driving_decision_conflicting_inputs(self):
        """Test decision-making logic under conflicting inputs."""
        self.fsd.traffic_signal = "green"
        self.fsd.pedestrian_nearby = True
        self.fsd.detect_obstacle(4)  # Simulate obstacle detection
        result = self.fsd.make_driving_decision()
        self.assertEqual(result, "STOP")  # Ensure system prioritizes safety.

    def test_logs_on_obstacle_detection(self):
        """Test that logs are generated correctly when an obstacle is detected."""
        with self.assertLogs('fsd_software', level='INFO') as log:
            self.fsd.detect_obstacle(4)
            self.assertIn('INFO:fsd_software:Obstacle detected at 4 meters.', log.output)

    def test_system_reset_with_config(self):
        """Test that reset works with custom configurations."""
        self.fsd.traffic_signal = "yellow"
        self.fsd.pedestrian_nearby = True
        self.fsd.detect_obstacle(2)  # Simulate obstacle detection
        self.fsd.reset_system(default_signal="yellow")
        self.assertEqual(self.fsd.traffic_signal, "yellow")  # Custom default
        self.assertFalse(self.fsd.pedestrian_nearby)
        self.assertFalse(self.fsd.obstacle_detected)
